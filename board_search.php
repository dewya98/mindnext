...
<style> 
    ...
    <content>
        ...
        <?php  }elseif($opt=="all"){
            $sql="select * from board where title like '%$keyword%' or content like '%$keyword%' or writer like '%$keyword%'" ;
            $rs=mysqli_query($conn,$sql);
            $total=mysqli_num_rows($rs);
            while($row=mysqli_fetch_array($rs)){ ?>
                <tr>
        ...
        <tr>
            <td colspan="5" align="center"><input type="button" value="게시판가기" class="button" onclick="location.href='board.php'"></td>
        </tr>
        </table>
    </content>

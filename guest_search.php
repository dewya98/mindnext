...
<style> 
    ...
    <content>
        ...
        <table>
          ...
          include "conn.php";
          if($opt=="writer"){
            $sql="select * from guest where writer like '%$keyword%'";
        ...
        <?php  }elseif($opt=="content"){
            $sql="select * from guest where content like '%$keyword%'";
            $rs=mysqli_query($conn,$sql);
            $total=mysqli_num_rows($rs);
            while($row=mysqli_fetch_array($rs)){ ?>
                <tr>
        ...
        <?php  }elseif($opt=="all"){
            $sql="select * from guest where content like '%$keyword%' or writer like '%$keyword%'" ;
            $rs=mysqli_query($conn,$sql);
            $total=mysqli_num_rows($rs);
            while($row=mysqli_fetch_array($rs)){ ?>
                <tr>
        ...
        <tr>
            <td colspan="5" align="center"><input type="button" value="게시판가기" class="button" onclick="location.href='guest.php'"></td>
        </tr>
        </table>
    </content>
    ...
    <script>
        function search(){
        location.href="guest_search.php?opt="+frm1.opt.value+"&keyword="+frm1.keyword.value;
        }
    </script>
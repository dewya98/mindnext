...
<style> 
    ...
    <content>
        ...
        <?php  }elseif($opt=="tag"){
            $sql="select * from notice where tag like '%$keyword%'";
            $rs=mysqli_query($conn,$sql);
            $total=mysqli_num_rows($rs);
            while($row=mysqli_fetch_array($rs)){ ?>
                <tr>
        ...
        <?php  }elseif($opt=="all"){
            $sql="select * from notice where title like '%$keyword%' or content like '%$keyword%' or writer like '%$keyword%'" ;
            $rs=mysqli_query($conn,$sql);
            $total=mysqli_num_rows($rs);
            while($row=mysqli_fetch_array($rs)){ ?>
                <tr>
        ...
        <tr>
            <td colspan="6" align="center"><input type="button" value="공지사항" class="button" onclick="location.href='notice.php'"></td>
        </tr>
        </table>
    </content>
